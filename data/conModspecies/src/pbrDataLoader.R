...
### find PB responsive species
all(colnames(pbr.08$c)==colnames(pbr.08$x));all(colnames(pbr.09$c)==colnames(pbr.09$x))

out.08 <- list()
for (i in 1:ncol(pbr.08$c)){
    out.08[[i]] <- t.test(pbr.08$c[,i],pbr.08$x[,i])
}
out.09 <- list()
for (i in 1:ncol(pbr.09$c)){
    out.09[[i]] <- t.test(pbr.09$c[,i],pbr.09$x[,i])
}

pii.08 <- unlist(lapply(out.08,function(x) x[[3]])) # pb interaction index
names(pii.08) <- colnames(pbr.08$c)
pii.09 <- unlist(lapply(out.09,function(x) x[[3]])) 
names(pii.09) <- colnames(pbr.09$c)
names(na.omit(pii.08[pii.08<=0.05]))
names(na.omit(pii.09[pii.09<=0.05]))
