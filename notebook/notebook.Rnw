...
\section{29 Jul 2014}
Re-run modularity for 2008 and 2009 control and exclusion with and
without pb.

And, calculate contribution to modularity.
...
Correlation between contribution to nestedness and affect of PB removal.

<<>>=
  ##get pb effect
  source('../src/pbr_load_data.R')
g09 <- geno.09[[1]]
pbe09 <- (pbr.09$c[,1] - pbr.09$x[,1])
cm <- dget('../results/cMods09.rda')[[1]]
hist(pbe09)
summary(aov(pbe09~g09))
plot(cm~pbe09,pch=19,ylab='z (Contribution to Modularity)',xlab='PB Removal Effect')
abline(lm(cm~pbe09))
cor.test(cm,pbe09)
cor.test(cm,I(pbe09/max(pbe09)))
plot(cm~I(pbe09/max(pbe09)))
...
Results:
\begin{itemize}
\item Modularity: Control = 0.17ns, Excluded = 0.20 p<0.001
\item PB Contribution: control z = -0.6467276, p = 0.20, exclusion z =
  -0.1391631, p = 0.48
\item Correlation between removal effect and contribution to
  modularity = 2.1677, df = 38, p-value = 0.03651
\end{itemize}

Discussion:
\begin{itemize}
\item Genetically based interaction between foundation species
  contributes to network modularity.
\item The presence of P. betae decreases modularity.
\item Modularity may be influenced by genotype.
\end{itemize}
...
rownames(n09c) <- rownames(n09x) <- g09

###Get modules
mod09c <- computeModules(n09c)
mod09x <- computeModules(n09x)
mem09c <- getMods(mod09c)
mem09x <- getMods(mod09x)

###

par(mfrow=c(1,2))
my.col <- topo.colors(6)
my.col[6] <- 'white'
colcl <- mem09c[[1]][order(mem09c[[1]])];colch <- mem09c[[2]][order(mem09c[[2]])]
colcl <- my.col[colcl];colch <- my.col[colch]
plotweb(n09c[order(mem09c[[1]]),order(mem09c[[2]])],text.rot=90,col.low=colcl,col.high=colch,method='normal',col.interaction='black',bor.col.interaction='black')
colxl <- mem09x[[1]][order(mem09x[[1]])];colxh <- mem09x[[2]][order(mem09x[[2]])]
colxl <- my.col[colxl];colxh <- my.col[colxh]
plotweb(n09x[order(mem09x[[1]]),order(mem09x[[2]])],text.rot=90,col.low=colxl,col.high=colxh,method='normal',col.interaction='black',bor.col.interaction='black')
...
P. betae contribution to modularity:
...
method <- 'r1'
y <- x
j <- (1:ncol(x))[colnames(x)=='pb']
for (nit in 1:n){
  print(nit)
  y[,j] <- commsimulator(x,method=method,thin=thin)[,j]
  sim[nit] <- slot(computeModules(y),name='likelihood')
}

###control
sim.c <- dget(file='../data/pb_cmod_control.rda')
obs.c <- dget(file='../data/pb_mumods_control.rda')
## obs.c <- 0
## for (i in 1:100){obs.c[i] <- slot(computeModules(sign(n09c)),name='likelihood')}
(mean(obs.c) - mean(sim.c))/sd(sim.c)
length(sim.c[sim.c<=mean(obs.c)])/length(sim.c)
##z = -0.6467276, p = 0.20

###exclusion
sim.x <- dget(file='../data/pb_cmod.rda')
obs.x <- dget(file='../data/pb_mumods_exclusion.rda')
## obs.x <- 0
## for (i in 1:100){obs.x[i] <- slot(computeModules(sign(n09x)),name='likelihood')}
## dput(obs.x,file='../data/pb_mumods_exclusion.rda')
(mean(obs.x) - mean(sim.x))/sd(sim.x)
length(sim.x[sim.x<=obs.x])/length(sim.x)
##z = -0.1391631, p = 0.48
...
###
mods <- dget('../results/sigMods09.rda')
mods[[1]][1:5]
mods[[2]][1:5]
cm <- dget('../results/cMods09.rda')
hist(cm[[1]])
hist(cm[[2]])
names(cm[[1]]) <- names(cm[[2]]) <- rownames(n09c)
plot(cm[[1]]~g09)
plot(cm[[2]]~g09)
plot(sqrt(I(cm[[2]]+abs(min(cm[[2]]))))~g09)
barplot(sort(cm[[1]]),las=2)
barplot(sort(cm[[2]]),las=2)
