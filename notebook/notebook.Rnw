...
#run modularity
##move simulated files up onto the server
##make a shell script for each one

###test to make sure similar results rae obtained using your null communities
nt08.c <- list()
for (i in 1:500){nt08.c[[i]] <- read.csv(paste('~/Desktop/null08/cpb/',i,sep=''))}
nt08.c <- lapply(nt08.c,nested)
nt08.c <- unlist(nt08.c)
plot(density(nt08.c),col='red')
nest08.c <- oecosimu(prn.08$c,nestfun=nested,method='r1',burnin=500)
test <- commsimulator(prn.08$c,method='r1',thin=500)
for (i in 1:500){test <- commsimulator(test,method='r1',thin=500)}
nested(test)
lines(density(nest08.c$oecosimu$simulated),col='green')
